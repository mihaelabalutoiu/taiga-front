...
class KanbanController extends mixOf(taiga.Controller, taiga.PageMixin, taiga.FiltersMixin, taiga.UsFiltersMixin)
    ...
    loadUserstories: () ->
        ...
        promise = @q.all(loadPromises).then (result) =>
            ...
            attributesToSkip = ["Attendees", "Start Time", "Timezone"]
            ...
            customAttributesPromises = userstories.map (us) =>
                @rs.customAttributesValues.userstory.get(us.id).then (customAttributesValues) =>
                    us.custom_attributes_values = _.map customAttributesValues._attrs.attributes_values, (value, key) =>
                        ...
                        if attributesToSkip.includes(name) or !value
                            return null
                        return {
                            id: key,
                            name: name,
                    ...
                    us.custom_attributes_values = us.custom_attributes_values.filter((attr) -> attr != null)
